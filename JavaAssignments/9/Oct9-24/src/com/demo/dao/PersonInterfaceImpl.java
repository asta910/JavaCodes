...
import com.demo.bean.Graduation;
import com.demo.bean.Masters;
...
public class PersonInterfaceImpl implements PersonInterface{
    ...
    //9. searchByName and sort
    public void searchByNameAndSort(){
        System.out.println("Enter name of student you want to search : ");
        String name = sc.nextLine();
            Set<Person> set = new TreeSet<Person>(Comparator.comparing(Person::getId));

        for (int i =0; i < personArray.size(); i++){
            if(personArray.get(i).getName().equals(name)){
                set.add(personArray.get(i));
            }
        }
        for (Person p : set){
